...
//		Collections.sort(bankAcctList, new BankAcctCompataror());

		int selectedOption = 0;
		Scanner opt = new Scanner(System.in);

		PaymentsFileOps fileOps = new PaymentsFileOps();
		try {
			usersList = fileOps.fileToUser();
			for (User u : usersList) {
				System.out.println(u.getUserId() + " " + u.getFirstName() + " " + u.getLastName() + " "
						+ u.getPhoneNum() + " " + u.getDateOfBirth() + " " + u.getCommunicationAddr());
			}
		} catch (Exception e) {
	...
	public static void addBankAccount() {
		...
		BankAcctCompataror baComp = new BankAcctCompataror();

		for(BankAccount b : bankAcctList) {
			if(baComp.compare(ba, baComp) ==0) {
				System.out.println("Bank Account Number "+ba.getBankAcctNumber()+" is already existing inthe system");
			}else {
				bankAcctList.add(ba);
			}
		}
